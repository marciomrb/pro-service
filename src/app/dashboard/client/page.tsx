        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            {!recentMessages || recentMessages.length === 0 ? (
              <p className="text-center py-12 text-muted-foreground bg-muted/5 rounded-3xl border-dashed border-2">
              ...
              recentMessages.map((msg: any) => (
                <Link
                  key={msg.id}
                  href={`/dashboard/client/messages/${msg.sender_id}`}
                >
                    ...
                    <div className="w-12 h-12 rounded-full bg-muted overflow-hidden flex-shrink-0">
                      <img
                        src={
                          ...
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(msg.sender?.full_name)}&background=0E5D91&color=fff`
                        }
                        alt="Avatar"
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {!favorites || favorites.length === 0 ? (
              <p className="text-center py-8 text-muted-foreground text-xs bg-muted/5 rounded-2xl border-dashed border-2">
                  ...
                  <div className="flex items-center justify-between p-3 rounded-2xl border bg-card hover:border-primary/30 transition-colors cursor-pointer group mb-2">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl overflow-hidden">
                        <img
                          src={
                            fav.provider.profiles.avatar_url ||
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(fav.provider.profiles.full_name)}&background=0E5D91&color=fff`
                          }
                          alt="Avatar"
                          className="w-full h-full object-cover"
                        />
                      ...
                      <div>
                        <h4 className="font-semibold text-sm group-hover:text-primary transition-colors">
                          {fav.provider.profiles.full_name}
                        </h4>
