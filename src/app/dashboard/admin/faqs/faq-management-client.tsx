  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <Button onClick={handleAddNew} className="w-full md:w-auto rounded-xl font-bold bg-primary shadow-lg shadow-primary/20">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/30 border-b border-border/50">
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground">Ordem</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground">Categoria</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground">Pergunta</th>
                <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/50">
              {filteredFaqs.length > 0 ? filteredFaqs.map((faq) => (
                <tr key={faq.id} className="hover:bg-muted/10 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <p className="font-bold text-sm text-foreground line-clamp-1">{faq.question}</p>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex gap-2">
                      ...
                      >
                        {isDeleting === faq.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                      </Button>
                    </div>
                  </td>
                </tr>
        ...
        <SheetContent side="right" className="sm:max-w-md border-l border-border/50">
          <form onSubmit={handleSave} className="flex flex-col h-full">
            ...
            <div className="flex-1 space-y-6 py-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="question" className="font-bold">Pergunta</Label>
                <Input 
                  id="question"
              ...
              <div className="space-y-2">
                <Label htmlFor="answer" className="font-bold">Resposta</Label>
                <Textarea 
                  id="answer"
              ...
              <div className="space-y-2">
                <Label htmlFor="sort_order" className="font-bold">Ordem de Exibição</Label>
                <Input 
                  id="sort_order"
                  type="number"
