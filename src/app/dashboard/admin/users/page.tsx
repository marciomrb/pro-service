...
import { Trash2, ShieldCheck, User, Wrench } from "lucide-react";
import { updateUserRole as _updateUserRole, deleteUser as _deleteUser } from "@/actions/admin-actions";

async function updateRole(userId: string, role: 'client' | 'provider' | 'admin'): Promise<void> {
  'use server'
...
export default async function AdminUsersPage() {
  ...
  const roleColors: Record<string, string> = {
    ...
    provider: "bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400",
    client: "bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400",
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight">Gerenciar Usuários</h1>
        <p className="text-muted-foreground mt-1">
          Lista de todos os usuários registrados — {users?.length || 0} usuário(s) encontrado(s).
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {users?.map((u: any) => (
          ...
          >
            {/* Role color bar */}
            <div className={`absolute top-0 left-0 w-full h-1 ${roleBarColors[u.role] || "bg-gray-400"}`} />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="overflow-hidden flex-1 min-w-0">
                <h3 className="font-bold text-lg leading-tight truncate" title={u.full_name}>
                  {u.full_name}
                </h3>
                <span
            ...
            <div className="flex flex-wrap gap-1.5">
              {(["client", "provider", "admin"] as const).map((r) => (
                <form
                  key={r}
                  action={updateRole.bind(null, u.id, r)}
                >
                  <button
                    type="submit"
                    disabled={u.role === r}
                  ...
                  >
                    {r === "client" && <User className="w-3 h-3 inline mr-1" />}
                    {r === "provider" && <Wrench className="w-3 h-3 inline mr-1" />}
                    {r === "admin" && <ShieldCheck className="w-3 h-3 inline mr-1" />}
                    {roleLabels[r]}
                  </button>
                </form>
              ))}
              ...
              {u.id !== user.id ? (
                <form action={removeUser.bind(null, u.id)}>
                  <button
                    ...
                    onClick={(e) => {
                      if (!confirm(`Tem certeza que deseja remover ${u.full_name}? Esta ação não pode ser desfeita.`)) {
                        e.preventDefault();
                      }
                    }}
                  >
                ...
                <p className="text-center text-xs text-muted-foreground py-1">Sua própria conta</p>
              )}
            </div>
          </Card>
        ))}
