...
import { addTicketResponse } from '@/app/support/actions'
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <Badge variant="outline" className="rounded-lg bg-background font-bold uppercase tracking-wider text-[10px]">
                  #{ticket.id.slice(0, 8)}
                </Badge>
              ...
              <h1 className="text-3xl font-black text-foreground">{ticket.subject}</h1>
            </div>
            <div className="text-right">
              <p className="text-sm text-muted-foreground font-medium">Aberto em</p>
              <p className="font-bold">{format(new Date(ticket.created_at), "d 'de' MMMM, yyyy", { locale: ptBR })}</p>
            </div>
          </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Altere o status deste chamado para organizar o fluxo de trabalho.</p>
                </div>
              </div>
              
              <form action={handleStatusUpdate} className="flex gap-3 w-full md:w-auto">
                <select 
                  name="status" 
                ...
                <Button type="submit" size="sm" className="rounded-xl font-bold bg-foreground hover:bg-primary text-white">
                  Atualizar
                </Button>
              </form>
            </div>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="space-y-6">
            {ticket.responses?.map((response: any) => (
              ...
              >
                <div className={`w-10 h-10 rounded-xl overflow-hidden shrink-0 border-2 ${response.is_admin_response ? 'border-primary shadow-lg shadow-primary/20' : 'border-background shadow-sm'}`}>
                  <img 
                    src={response.profiles?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${response.profiles?.full_name}`} 
                    alt="Avatar" 
                    className="w-full h-full object-cover"
                  />
                </div>
                <div className={`flex-1 space-y-1 ${response.is_admin_response ? 'items-end' : ''}`}>
                  <div className={`flex items-center gap-2 mb-1 ${response.is_admin_response ? 'flex-row-reverse' : ''}`}>
                    <span className="font-bold text-sm">{response.profiles?.full_name}</span>
                    {response.is_admin_response && (
                      <Badge className="bg-primary text-white text-[10px] h-4 px-1.5 font-black uppercase">Suporte</Badge>
                    )}
                    <span className="text-[10px] text-muted-foreground">{format(new Date(response.created_at), "HH:mm")}</span>
                  </div>
            ...
            {(!ticket.responses || ticket.responses.length === 0) && (
              <div className="text-center py-8 bg-muted/20 rounded-3xl border border-dashed border-muted/50">
                <p className="text-sm text-muted-foreground font-medium">Aguardando primeira resposta da nossa equipe...</p>
              </div>
            )}
          </div>
          ...
          {ticket.status !== 'closed' && (
            <form action={handleResponse} className="mt-12 space-y-4">
              ...
              <Button type="submit" className="w-full h-12 rounded-xl bg-foreground hover:bg-primary transition-all duration-300 font-bold shadow-lg shadow-primary/10">
                Enviar Resposta
              </Button>
            </form>
          )}
