...
import { Check, ShieldCheck, Zap, CreditCard, Ticket, Loader2, AlertCircle, MessageSquare, Sparkles, X } from 'lucide-react'
import { createProviderSubscription, requestCoupon } from '@/actions/subscription-actions'
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (user) {
        ...
        if (profile) {
          const provider = Array.isArray(profile.provider_profiles) ? profile.provider_profiles[0] : profile.provider_profiles
          if (provider?.cpf_cnpj) {
            setHasBillingInfo(true)
            setBillingInfo({ cpf_cnpj: provider.cpf_cnpj, phone: provider.phone || '' })
          }
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-4 space-y-12">
      ...
      >
        ...
        <h1 className="text-4xl md:text-5xl font-black tracking-tighter bg-gradient-to-br from-foreground to-foreground/60 bg-clip-text text-transparent">
          Plano Pro para Prestadores
        </h1>
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Aumente sua visibilidade, conquiste mais clientes e profissionalize seu negócio com ferramentas exclusivas.
        </p>
      </motion.div>
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-8">
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { title: "Selo de Verificado", desc: "Destaque-se com o selo azul de confiança.", icon: <ShieldCheck className="w-5 h-5 text-blue-500" /> },
              { title: "Prioridade na Busca", desc: "Apareça antes de outros prestadores.", icon: <Zap className="w-5 h-5 text-yellow-500" /> },
              { title: "Feed Social Ilimitado", desc: "Poste fotos e vídeos dos seus serviços.", icon: <Sparkles className="w-5 h-5 text-purple-500" /> },
              { title: "Chat em Tempo Real", desc: "Negocie diretamente com o cliente.", icon: <MessageSquare className="w-5 h-5 text-green-500" /> },
            ].map((benefit, i) => (
              <motion.div 
                key={i}
          ...
          <Card className="p-6 bg-primary/5 border-primary/20 rounded-3xl overflow-hidden relative">
            ...
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground italic">Não prendemos você com contratos. Sua evolução depende do seu resultado.</p>
              </div>
            </div>
          </Card>
        </div>
                ...
                <Card className="p-8 rounded-[2.5rem] border-primary/20 shadow-2xl bg-card/80 backdrop-blur-xl">
                  <div className="space-y-6">
                    <div className="text-center space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">Precisamos desses dados para gerar sua cobrança no Asaas.</p>
                    </div>
                    ...
                    <form onSubmit={handleUpdateBilling} className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          onChange={(e) => setBillingInfo({...billingInfo, cpf_cnpj: e.target.value})}
                          required
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground ml-1">Telefone (WhatsApp)</label>
                        <Input 
                ...
                <Card className="p-8 rounded-[2.5rem] border-primary shadow-2xl bg-card relative overflow-hidden ring-4 ring-primary/5">
                  ...
                  <div className="space-y-8">
                    <div className="space-y-2">
                      ...
                      <p className="text-muted-foreground">O investimento que se paga no primeiro serviço.</p>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="space-y-3">
                        ...
                        <p className="text-[10px] text-muted-foreground ml-1">
                          Não tem um cupom? <button onClick={() => setIsRequestingCoupon(true)} className="text-primary font-bold hover:underline">Solicite um desconto aqui</button>
                        </p>
                      </div>
                      ...
                      {subscriptionStatus === 'PENDING' || subscriptionStatus === 'ACTIVE' ? (
                        <div className="p-4 rounded-2xl bg-green-500/10 border border-green-500/20 text-green-700 text-center space-y-2">
                          ...
                          <p className="text-xs">Verifique seu e-mail ou o portal do Asaas para o pagamento.</p>
                        </div>
                      ) : (
                        <Button 
            ...
            >
              <Card className="p-6 border-primary shadow-2xl">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setIsRequestingCoupon(false)}>
                    <X className="w-5 h-5" />
                  </Button>
                </div>
                ...
                <form action={async (formData) => {
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Código que você gostaria</label>
                    <Input 
                      name="code" 
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Por que você merece esse desconto?</label>
                    <textarea 
                      name="reason"
