...
export function VerificationReview({ documents, providerId }: VerificationReviewProps) {
  ...
  return (
    <div className="grid gap-6">
      {documents.map((doc) => (
        <Card key={doc.id} className="p-6 rounded-3xl border-border/50 shadow-sm overflow-hidden flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Enviado em {new Date(doc.created_at).toLocaleDateString('pt-BR')}</p>
                </div>
              </div>
            ...
            {doc.status === 'rejected' && doc.rejection_reason && (
              <div className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10 flex gap-3 text-sm text-red-600">
                ...
                <p><strong>Motivo da rejeição:</strong> {doc.rejection_reason}</p>
              </div>
            )}
          </div>
            ...
            {doc.status === 'pending' ? (
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-medium text-muted-foreground ml-1">Motivo (se rejeitar)</label>
                  <Input 
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    {loading === doc.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <XCircle className="w-4 h-4 mr-2" />}
                    Rejeitar
                  </Button>
                  <Button 
                  ...
                  >
                    {loading === doc.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4 mr-2" />}
                    Aprovar
                  </Button>
                </div>
              </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-4 space-y-2">
                <p className="text-sm text-muted-foreground font-medium">Ação concluída</p>
                ...
                <AnimatePresence mode="wait">
                  {confirmReset === doc.id ? (
                    <motion.div 
                      ...
                      className="space-y-2"
                    >
                      <p className="text-[10px] text-red-500 font-bold uppercase tracking-wider">Tem certeza?</p>
