...
import { Calendar as CalendarIcon, Clock, ChevronRight, MapPin, Navigation } from 'lucide-react'
import { createBooking } from "@/actions/booking-actions"
import { format } from 'date-fns'
import { useGeolocation } from '@/hooks/use-geolocation'

export default function BookingForm({ providerId, providerName }: { providerId: string, providerName: string }) {
    ...
    <Card className="p-6 rounded-3xl border-primary/10 shadow-xl space-y-6 overflow-hidden bg-card/50 backdrop-blur-xl">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {[1, 2, 3].map((s) => (
            <div key={s} className={`w-2 h-2 rounded-full transition-all duration-300 ${step >= s ? 'bg-primary w-4' : 'bg-muted'}`} />
          ))}
        </div>
      </div>
        ...
        {step === 2 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="flex gap-2">
              <Button variant="ghost" className="flex-1 rounded-2xl h-12 font-bold" onClick={() => setStep(1)}>
                Back
              </Button>
              <Button 
        ...
        {step === 3 && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="space-y-3">
              <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10 space-y-3">
                <Button 
                  ...
                  disabled={geoLoading}
                >
                  {geoLoading ? "Getting Location..." : location ? "Current Location Added" : "Use Current Location"}
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-background px-2 text-muted-foreground">Or add address</span>
                </div>
              </div>
            ...
            <div className="flex gap-2 pt-4">
              <Button variant="ghost" className="flex-1 rounded-2xl h-12 font-bold" onClick={() => setStep(2)}>
                Back
              </Button>
              <Button 
                className="flex-[2] rounded-2xl h-12 text-lg font-bold shadow-lg bg-primary hover:bg-accent"
      ...
      <p className="text-[10px] text-center text-muted-foreground">
        Your request will be sent to <strong>{providerName}</strong> for approval.
      </p>
    </Card>
  )
}
